...
const totalContributions= GAMES_JSON.reduce((acc,games)=>{ return acc + games.backers;}, 0);
...
const totalRaised= GAMES_JSON.reduce((acc,games)=>{return acc+games.pledged},0)
...
function filterFundedOnly() {
    ...
    let funded = GAMES_JSON.filter((games)=> {return games.pledged >= games.goal})
    console.log(funded)
...
let numberUnfundedGames=GAMES_JSON.filter((games)=>games.pledged<games.goal).length

// create a string that explains the number of unfunded games using the ternary operator
const displayStr= `
    A total of $${totalContributions.toLocaleString('en-US')} has been raised for ${GAMES_JSON.length}. Currently, ${numberUnfundedGames} remains unfunded. We need your help to fund thee amazing games!
    `

// create a new DOM element containing the template string and append it to the description container
//one or more game is unfunded
